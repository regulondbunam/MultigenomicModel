# -*- coding: utf-8 -*-
# NAME [ParserJsonMarkdownGene_v0.4.py]   Version [1.0]
# AUTHOR  kevin alquicira y shirley alquicira
# CREATED (2019-10)
# USAGE python ParserJsonMarkdownGene_v0.4.py -f ../data/gene_schema_annotation.json -s ../config/format_style.json -o ../results/
...
for key in data['validator']['$jsonSchema']['properties'].keys():
  ...
  for key_first_level in keys_first_level:
    if ('collections.OrderedDict' not in str(type(data['validator']['$jsonSchema']['properties'][key][key_first_level])) and (key_first_level in 'bsonType')):
